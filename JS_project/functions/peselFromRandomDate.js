...
function getPeselFromRandomDate() {
    ...
    if (newYear == 1) {
        const peselRD = getNumbers(shortDate[5], shortDate);
        document.getElementById('newRP').innerHTML += peselRD;
        const message = genderMessage(peselRD);
    } else {
        if (newMonth != 0) {
            const peselRD = getNumbers(3, shortDate);
            document.getElementById('newRP').innerHTML += peselRD;
            const message = genderMessage(peselRD);
        } else {
            const peselRD = getNumbers(2, shortDate);
            document.getElementById('newRP').innerHTML += peselRD;
            const message = genderMessage(peselRD);
        }
    }
